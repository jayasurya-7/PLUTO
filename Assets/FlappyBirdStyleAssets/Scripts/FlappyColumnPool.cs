...
public class FlappyColumnPool : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!setup)
        {
            ...
            FlappyColumnPool.instance.difficultyLevel = 1;
            ...
            {
                g.transform.localPosition = new Vector3(g.transform.localPosition.x, Mathf.Clamp((7 - difficultyLevel) * .25f, 0, 0), 0);
            }
    ...
    public void spawnColumn()
    {
        if (!FlappyGameControl.instance.gameOver && prevSpawnTime > 2)
        {
            ...
            columns[CurrentColumn].transform.position = new Vector2(BirdControl.rb2d.transform.position.x + spawnXposition, 0);
